...
class MyFileDetector : TruffleFile.FileTypeDetector {
    override fun findMimeType(file: TruffleFile?): String? {
        return file?.name?.endsWith(".ml")?.let { ml -> if(ml) { return "application/mylang" } else { null }}
    }
...
@TruffleLanguage.Registration(id = "mylang", name = "ml", defaultMimeType = "application/ml", characterMimeTypes = ["application/ml"], contextPolicy = TruffleLanguage.ContextPolicy.SHARED, fileTypeDetectors = [ MyFileDetector::class ])
...
fun run() {
    ...
    while(running) {
        ...
        val program = Program(line!!.split(' ').map(String::trim).filter { !it.isEmpty() }.map(parser::parse).toTypedArray(), frameDescriptor)
...
//    parser.parseListeners.add()
    ...
    var read = SLReadLocalVariableNodeGen.create(counter)
